...
class TranslationDataset_cl(data.Dataset):
    """Defines a dataset for machine translation."""
    ...
    def __init__(self, path, exts, fields, difficulty_path, **kwargs):
        """Create a TranslationDataset given paths and fields.
        ...
        """
        if not isinstance(fields[0], (tuple, list)):
            fields = [('src', fields[0]), ('trg', fields[1]), ('difficulty', fields[2])]
        ...
        with io.open(src_path, mode='r', encoding='utf-8') as src_file, \
                io.open(trg_path, mode='r', encoding='utf-8') as trg_file, \
                    io.open(difficulty_path, mode='r', encoding='utf-8') as difficulty_file:
            for src_line, trg_line, d in zip(src_file, trg_file, difficulty_file):
                src_line, trg_line = src_line.strip(), trg_line.strip()
                d = float(d)
                if src_line != '' and trg_line != '':
                    examples.append(data.Example.fromlist(
                        [src_line, trg_line, d], fields))

        super(TranslationDataset_cl, self).__init__(examples, fields, **kwargs)
