...
class Iterator_cl(object):
    ...
    def init_epoch(self, competence):
        """Set up the batch generator for a new epoch."""
        ...
        self.create_batches(competence)
    ...
    def create_batches(self, competence):
        self.batches = batch_cl(self.data(), self.batch_size, self.batch_size_fn)
    ...
    def __iter__(self):
        while True:
            self.init_epoch(self.competence)
            for idx, minibatch in enumerate(self.batches):
                # fast-forward if loaded from state
                if self._iterations_this_epoch > idx:
                    continue
...
class BucketIterator_cl(Iterator_cl):
    """Defines an iterator that batches examples of similar lengths together.
    Minimizes amount of padding needed while producing freshly shuffled
    batches for each new epoch. See pool for the bucketing procedure used.
    """
    ...
    def create_batches(self, competence):
        if self.sort:
            self.batches = batch_cl(self.data(), self.batch_size,
                                    competence,
                                    self.batch_size_fn)
        else:
            self.batches = pool_cl(self.data(), self.batch_size,
                                   self.sort_key,
                                   competence,
                                   self.batch_size_fn,
                                   random_shuffler=self.random_shuffler,
...
def batch_cl(data, batch_size, competence, batch_size_fn=None):
    """Yield elements from data in chunks of batch_size."""
    if batch_size_fn is None:
        def batch_size_fn(new, count, sofar):
            return count
    ...
    if competence != 1:
        for i, ex in enumerate(data):
            if ex.difficulty > competence:
                max_index = i - 1
                break
    else:
        max_index = len(data) - 1
    iterations_count, iterations_num = 1, 1
    total_difficulty, loop_count = 0, 0
    while True:
        selected_index = random.randint(0, max_index)
        ex = data[selected_index]
        ...
        if size_so_far == batch_size:
            ...
            minibatch, size_so_far = [], 0
            iterations_count += 1
            if iterations_count >= iterations_num:
                print("Curriculum metrics at this epoch: ave_difficulty = {0}, competence = {1}".format(total_difficulty / loop_count, competence))
                break
        elif size_so_far > batch_size:
            yield minibatch[:-1]
            minibatch, size_so_far = minibatch[-1:], batch_size_fn(ex, 1, 0)
...
def pool_cl(data, batch_size, key, competence, batch_size_fn=lambda new, count, sofar: count,
         random_shuffler=None, shuffle=False, sort_within_batch=False):
    """Sort within buckets, then batch, then shuffle batches.
    Partitions data into chunks of size 100*batch_size, sorts examples within
    each chunk using sort_key, then batch these examples and shuffle the
    ...
    if random_shuffler is None:
        random_shuffler = random.shuffle
    for p in batch_cl(data, batch_size * 100, competence, batch_size_fn):
        p_batch = batch(sorted(p, key=key), batch_size, batch_size_fn) \
            if sort_within_batch \
            else batch(p, batch_size, batch_size_fn)
        if shuffle:
