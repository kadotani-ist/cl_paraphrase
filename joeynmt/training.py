...
from joeynmt.data import load_data, make_data_iter, load_data_cl, make_data_iter_cl
from joeynmt.builders import build_optimizer, build_scheduler, \
    build_gradient_clipper
from joeynmt.prediction import test

from joeynmt import competence_metrics
...
class TrainManager:
    ...
    def __init__(self, model: Model, config: dict) -> None:
        ...
        # curriculum learning
        self.cl_flag = train_config.get("curriculum_learning", False)
        if self.cl_flag:
            self.competence_parameter = train_config.get("competence_parameter", 1)
            self.competence = 0.01
            self.epoch_loss = 0
            self.epoch_loss_before = 0
    ...
    def train_and_validate(self, train_data: Dataset, valid_data: Dataset) \
        ...
        for epoch_no in range(self.epochs):
            ...
            if self.cl_flag:
                if epoch_no != 0:
                    self.epoch_loss_before = self.epoch_loss
                    self.epoch_loss = epoch_loss
                self.competence = competence_metrics.steps_based(self.steps)
            epoch_loss = 0

            if self.cl_flag and self.competence >= 1:
                train_iter = make_data_iter(train_data,
                                            batch_size=self.batch_size,
                                            batch_type=self.batch_type,
                                            train=True, shuffle=self.shuffle)
                self.cl_flag = False
                self.logger.info(
                    'Curriculum learning ended and shifted to no curriculum learning.')
            if self.cl_flag:
                train_iter = make_data_iter_cl(self.competence,
                                               train_data,
                                               batch_size=self.batch_size,
                                               batch_type=self.batch_type,
                                               train=True, shuffle=self.shuffle)
            ...
            for i, batch in enumerate(iter(train_iter)):
                ...
                if self.steps % self.validation_freq == 0 and update:
                    ...
                    # execute early stopping processes
                    if self.minimize_metric:
                        if ckpt_score > last_ckpt_score:
            ...
            if early_stopping_count >= self.early_stopping_patience:
                self.logger.info(
                    'Training ended since early stopping patience %d was reached.',
            ...
            if self.steps >= 100000:
                self.logger.info(
                    'Training ended since steps %d was reached.',
                    self.steps)
                break
...
def train(cfg_file: str) -> None:
    ...
    cl_flag = train_config.get("curriculum_learning", False)
    if cl_flag:
        train_data, dev_data, test_data, src_vocab, trg_vocab = load_data_cl(
            data_cfg=cfg["data"])
    else:
